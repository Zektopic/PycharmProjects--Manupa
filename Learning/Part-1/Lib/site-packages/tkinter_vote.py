...
def btnAddClick():
    ...
    if names:
        ...
        for name in names:
            # 已经存在的选项自动忽略
            sqlExists = 'SELECT COUNT(*) FROM names WHERE name=?'
            if Common.getData(sqlExists, (name,))[0][0] == 0:
                sql = 'INSERT INTO names values(?)'
                Common.doSql(sql, (name,))
...
def btnVoteClick():
    ...
    sqlVoted = 'SELECT COUNT(*) FROM votes WHERE voter=?'
    count = Common.getData(sqlVoted, (voter,))[0][0]
    if count != 0:
    ...
    for index, name in enumerate(Common.lstNames):
        if eval('Common.name'+str(index)).get() == 1:
            ...
            Common.doSql(sql, (voter, name['text'], str(datetime.datetime.now())[:19]))
            selected = True
    if selected:
